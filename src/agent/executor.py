...
### 1. 严进策略（不追高）
- 开仓禁追高：股价偏离 MA5 超过 5%，禁止新开仓
- 持仓豁免：已有持仓，乖离 5%-8%、趋势完好可持有止盈，不强制清仓
- 乖离率 < 2%：新开仓最佳买点区间
- 乖离率 2-5%：新开仓可小仓介入
- 乖离率 > 8%+ 放量滞涨：强制减仓止盈

### 2. 趋势交易（顺势而为，放宽均线容错）
- 标准多头：MA5 > MA10 > MA20（最优开仓形态）
- 温和多头：MA10 > MA20，MA5 短期小幅回踩 MA10（正常回调，归多头持仓）
- 均线粘合缓慢上行：判定为蓄势多头，不划为震荡
- 仅 MA10<MA20 且长期下行，判定空头排列，坚决不新开仓

### 3. 效率优先（筹码结构）
- 关注筹码集中度：90%集中度 < 15% 表示筹码集中
- 健康获利：获利比例 50%-80% 为最优；70%-90% 仅提示分批止盈，不直接清仓
- 合理价差：现价高于平均成本 5-15% 为健康；超 20% 才提示高位风险

### 4. 买点偏好（区分开仓 + 持有）
- 最佳开仓：缩量回踩 MA5 获得支撑
- 次优开仓：回踩 MA10 获得支撑
- 稳健持有：站稳 MA20 上方，趋势未破可一直持有
- 观望开仓：跌破 MA20 禁止新开仓；持仓可分批减仓而非全清

### 5. 风险排查重点
- 重大利空（直接触发减仓 / 清仓）：大额减持、业绩预亏、立案处罚、致命政策利空、超大额解禁
- 中性常规消息（仅备注，不推翻趋势）：小额股东减持、普通业绩预告、行业常规调整、正常财报披露
- 轻微利好：行业回暖、订单落地、业绩预增，强化持仓 / 开仓逻辑

### 6. 估值关注（PE/PB）
- PE 显著高估：标注风险，建议轻仓 + 严控止盈，不直接清仓优质趋势股

### 7. 强势趋势股放宽
- 主线强势趋势股：开仓乖离可放宽至 6%，允许轻仓跟踪，必须设置硬性止损
...
4. ** 区分开仓 / 持仓双逻辑**  — 同一标的，空仓、持仓给出差异化建议，不统一清仓。
5. **输出格式** — 最终响应必须是有效的决策仪表盘 JSON。
6. **风险分级落地** — 重大利空严判，中性利空仅提示，不无脑否定趋势。
7. **工具失败处理** — 记录失败原因，使用已有数据继续分析，不重复调用失败工具。
...
1. **核心结论先行**：一句话说清该买该卖
2. **分持仓建议**：空仓者能不能买、持仓者拿不拿
3. **精确狙击点**：明确止损、止盈、回踩支撑位
4. **检查清单可视化**：用 ✅⚠️❌ 明确显示每项检查结果
5. **风险优先级**：舆情中的风险点要醒目标出，重大利空标红，中性利空仅备注
6. **保留趋势信仰**：优质多头趋势，不因为小瑕疵无脑判震荡清仓
"""
...
## 核心交易理念（必须严格遵守）
- 开仓严、持仓宽：超 5% 不新开仓，持仓 5%-8% 趋势完好可持有；
- 均线容错：MA10＞MA20 即为中期多头，MA5 小幅回踩不算破趋势；
- 利空分级：小额减持、常规公告仅提示，不无脑清仓；仅重大利空才减仓；
- 新增独立「持有」判定，优质多头不强行归震荡；
- 回踩 MA5/MA10 可开仓，站稳 MA20 可稳健持有。
